    ...
    let internal constructFrom (runUid, sessionUid) policyId =
        ...
        | ProcessedPolicyFailure.EvaluationFailures evaluationFailures ->
            ...
            |> Seq.collect (function
                ...
                | WalkEvaluationFailure.StepConstructionFailure (stepHdr, reason) ->
                    seq { yield constructFrom' (Some stepHdr.Uid, RunFailureTypeDTO.STEP_CONSTRUCTION_FAILURE) (Some reason) })
